...
function transformToLis(obj){
  ...
  return newKeys.map(key => `<li>${key}: ${obj[key]}</li>`);
}
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for(let i=0; i<input.length; i++){
    ...
    let mew = currentArray.map(number => {
      if((number % 5 ===0) && (typeof number ==='number')){
        let currentNum = Math.pow(2,number);
        return currentNum;
        }
    }); powerArray.push(mew.filter(num => (typeof num ==='number')))
  } return powerArray;
};
...
let findMaleAndFemale = (data) => {
  let MF = data.filter(person => (person.gender === 'male') || (person.gender === 'female'));
  let names = MF.map(person => person.name);
