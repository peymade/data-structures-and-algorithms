...
const addValues = (arr) => {
 let valueArray = arr.reduce((acc, num)=>{
   acc = acc + num;
   return acc;
 }, 0); return valueArray;
};
...
const addPurchases = (arr) => {
  let priceArray = arr.reduce((acc,price) => {
    return acc + price.purchasePrice;
  },0); return priceArray;
};
...
const reversedString = (str) => {
  ...
  const reverseString = stringArray.reduce((reverse,char) => {
    return char + reverse
